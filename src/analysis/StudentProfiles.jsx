...
export default function StudentProfiles({ allStudents, exams }) {
  ...
  const studentHistory = useMemo(() => {
    ...
    return students
      .filter(s => {
        if (selectedStudent.ic && s.ic) return s.ic === selectedStudent.ic;
        return s.name === selectedStudent.name;
      })
                ...
                {[
                  ...
                  return (
                    <div key={c.name} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                      <p style={{ fontFamily: font, fontSize: 12, fontWeight: 500, width: 70, textAlign: "right", margin: 0 }}>{c.name}</p>
                      <div style={{ flex: 1, height: 24, background: "#F0F0EC", borderRadius: 6, overflow: "hidden", position: "relative" }}>
                        <div style={{ width: `${(sv / 90) * 100}%`, height: "100%", background: diff >= 0 ? colors.accent : colors.band1, borderRadius: 6, transition: "width 0.6s ease" }} />
        ...
        <Card>
          <div style={{ textAlign: "center", padding: "30px 20px" }}>
            ...
            <p style={{ fontFamily: font, fontSize: 14, color: colors.textMuted, margin: 0 }}>
              Type a student's name or IC number above to view their profile
            </p>
          </div>
        </Card>
      )}
